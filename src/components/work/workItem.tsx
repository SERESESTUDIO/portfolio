...
export const WorkItem = ({configuration=sampleWorkConfig}) => {
  ...
  return (
    <a href={`/work/${configuration.id}`} target='_blank' key={configuration.title} className={`grid-item ${configuration.size}`} onPointerEnter={()=>setIsOver(true)} onPointerLeave={()=>setIsOver(false)}>
